...
This is accomplished by first running tests in the Playdate emulator and saving screenshots. These are considered the _expected_ results. Then tests are run in Love2d/Playdate and also saving screenshots. These are considered the _actual_ results. Then these two sets of screenshots are compared for differences to determine wether each test passes or fails.
...
5. When the tests finish running on Playdate, the simulator will close and you should have screenshots in `tests/src/images/expected/`.
...
## Adding Tests
The `tests/src` folder contains a Playbit project (aka the test project) that is runnable on both Playdate and Love2d. Deeper in this folder is the subfolder `tests/src/suites` which contains the actual test suites.